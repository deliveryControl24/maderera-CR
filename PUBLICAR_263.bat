...
echo ============================================
echo  PUBLICAR v2.6.3 - EXE nuevo (1 clic)
echo ============================================
echo.
...
git add config_paths.py app.py main.py modulos.py updater.py database.py excel_export.py themes.py utils.py cargar_datos_prueba.py actualizador.py version.json updates/latest.zip PUBLICAR_263.bat PUBLICAR_262.bat PUBLICAR_261.bat PUBLICAR_260.bat PUBLICAR_251.bat SUBIR.bat build_exe.bat empaquetar_dist.bat empacotar_update.py AGENTS.md COMANDOS_GITHUB.txt .gitignore
git commit -m "v2.6.3: dialogo legible + fixes" || echo (ya estaba commiteado)
...
git commit -m "v2.6.3: EXE dialogo legible + ACTUALIZADOR" || echo (EXE ya era igual)
git push origin main
if errorlevel 1 (
...
echo ============================================
echo  LISTO
echo  - EXE 2.6.3 con dialogo legible (changelog y botones)
echo  - version.json 2.6.3 en GitHub
echo  - paquete_pino\ actualizado
echo.
echo En clientes: cerrar PINO, abrir INICIAR.bat
echo o doble clic ACTUALIZAR - debe decir 2.6.3
