...
Si hay EXE Windows, ademas rebuild + subir `updates/PINO_SYSTEM.exe` (ej. `PUBLICAR_263.bat`) y `updates/ACTUALIZADOR.exe` si aplica.
...
- Build EXE: solo en Windows local (`PUBLICAR_263.bat`); PyInstaller no sirve en el share de Parallels ni en Mac
