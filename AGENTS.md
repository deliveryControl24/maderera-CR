...
Si hay EXE Windows, ademas rebuild + subir `updates/PINO_SYSTEM.exe` (ej. `PUBLICAR_264.bat`) y `updates/ACTUALIZADOR.exe` si aplica.
...
- Build EXE: solo en Windows local (`PUBLICAR_264.bat`); PyInstaller no sirve en el share de Parallels ni en Mac
